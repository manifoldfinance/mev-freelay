...
const { getURLs } = require("/lib/utils.js")

function handler(w, r) {
  const e = getURLs(envs.network)
  const data = stats(r)

  mustache.render("index", {
    totalValidators: data.totalValidators,
    registeredValidators: data.registeredValidators,
    latestSlot: data.latestSlot,
    deliveredCount: data.delivered.length,
    totalDelivered: data.totalDelivered,
    delivered: data.delivered,
    pubkey: envs.pubkey,
    network: envs.network,
    etherscanUrl: e.etherscan,
    beaconchainUrl: e.beaconchain,
  })
}
